...
struct FramebufferInfo {
    ...
    UINT32 Height;
    UINT32 Pitch;
    UINT64 MapSize;   // memory map info for the kernel PMM
    UINT64 DescSize;
};
...
extern "C" EFI_STATUS EFIAPI efi_main(EFI_HANDLE ImageHandle, EFI_SYSTEM_TABLE *SystemTable) {
    ...
    // copy the memory map to 0x80000 for the kernel's PMM
    // (boot services are gone now, that memory is ours)
    UINT8* map_dst = (UINT8*)0x80000;
    UINT8* map_src = (UINT8*)map;
    for (UINTN i = 0; i < map_size; i++) map_dst[i] = map_src[i];
    fb_info->MapSize = map_size;
    fb_info->DescSize = desc_size;
